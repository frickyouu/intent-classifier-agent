...
def get_user_choice():
    print("\nChoose input method:")
    print("1. Upload a file (PDF/JSON/EML)")
    print("2. Upload text of email")
    print("3. Enter JSON manually")
    print("4. Exit")
    return input("Your choice (1-4): ")

def handle_file_upload():
    file_path = input("Drag file here or enter path: ").strip('"')
    if not os.path.exists(file_path):
        print("Error: File not found!")
        return None
    if file_path.lower().endswith('.pdf'):
        with open(file_path, 'rb') as f:
            return f.read()  # Return binary PDF data
    elif file_path.lower().endswith('.eml'):
        with open(file_path, 'rb') as f:
            return f.read()  # Return binary email data
    else:
        with open(file_path, 'r', encoding='utf-8') as f:
            return f.read()  # Return text for JSON or other
  # Return text for JSON/emails
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if choice == "1":  # File upload
            input_data = handle_file_upload()
            if input_data:
                result = classifier.process_input(input_data)
                print("\nClassification Result:")
                print(result)
                
        elif choice == "2":  # Direct text/email
            text = input("Paste your text/email:\n")
            result = classifier.process_input(text)
            print("\nClassification Result:")
            print(result)
            
        elif choice == "3":  # JSON input
            json_str = input("Enter JSON:\n")
            result = classifier.process_input(json_str)
            print("\nClassification Result:")
            print(result)
